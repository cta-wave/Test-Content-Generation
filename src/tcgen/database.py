...
class Database:
    ...
    def test_id(tc:TestContent):
        if 'splice_main' in tc.test_id:
            ...
            return f't{tc.test_id.replace('_enc', '-cenc')}' if tc.encryption else f't{tc.test_id}'
    ...
    def format_entry(m:Mezzanine, t:TestContent, batch_dir:str):
        ...
        data = {
            ...
            'representations': [t.get_representation(m).to_json()],
            'segmentDuration': str(t.get_seg_dur(m)),
            'fragmentType': t.fragment_type.value,
